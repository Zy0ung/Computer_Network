...
int main(int argc, char *argv[]) {
    ...
    while (1) { // 무한루프를 돌면서 메시지 송수신
        ...
        rcvSize = recvfrom(sock, buffer, BUFFER_SIZE, 0, (struct sockaddr *)&cliAddr, &cliAddrLen); // sock 소켓을 통해 buffer로 메시지 수신
    if (rcvSize < 0) {
    ...
    sentSize = sendto(sock, buffer, strlen(buffer)+1, 0, (struct sockaddr *)&cliAddr, sizeof(cliAddr)); // sock 소켓을 통해 변환한 데이터 전송
